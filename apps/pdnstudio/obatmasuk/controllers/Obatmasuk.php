...
class Obatmasuk extends CI_Controller
{
	...
	function add()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$register_data['om_tanggal'] 		= htmlspecialchars($this->input->post('tanggal', true));
			$register_data['om_jumlah'] 		= htmlspecialchars($this->input->post('jumlah', true));
			$register_data['om_obatkode'] 		= str_replace(' ', '', $kode_obat);
			$register_data['om_create'] 		= date('Y-m-d H:i:s');
	...
	function edit()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->data['dtobat'] = $this->M_najzmi->db_get();
			$om = $this->M_najzmi->edit($_id);
			$this->data['kode_obat'] = $om->om_obatkode;
			$this->data['nama_obat'] = $om->obat_nama;
			$this->data['id'] = [
				'name' 			=> 'id',
				'id' 			=> 'id',
				'type' 			=> 'hidden',
				'required' 		=> 'required',
				'value' 		=> base64_encode($om->id_om),
			];
			$this->data['tanggal'] = [
				'name' 			=> 'tanggal',
				'id' 			=> 'tanggal',
	...
	function data_json()
	{
		...
		$tabel = 'v_j_obatmasuk';
		$column_order = array('', 'om_obatkode', 'om_tanggal', 'om_jumlah');
		$column_search = array('om_obatkode', 'om_tanggal', 'om_jumlah');
		$order = array('id_om' => 'DESC');
		...
		foreach ($list as $pDn) {
			...
			$row[] = $pDn->obat_nama;
			$row[] = $pDn->om_tanggal;
			$row[] = $pDn->om_jumlah.' '.$pDn->obat_satuan;
			$row[] = '<div class="btn-group">
								  <a href="'.$this->ClassNama().'/edit/' . base64_encode($pDn->id_om) . '"  class="btn btn-success btn-sm">Edit</a>
								  <a href="'.$this->ClassNama().'/hapus/' . base64_encode($pDn->id_om) . '" class="btn btn-danger btn-sm tombol-hapus">Hapus</a>
								</div>';
